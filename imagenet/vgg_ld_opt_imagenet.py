...
cfg = {
    ...
    'VGG13': [64, 64, 'M', 128, 128, 'M', 256, 256, 'M', 512, 512, 'M', 512, 512, 'A'],
    'VGG16': [64, 64, 'M', 128, 128, 'M', 256, 256, 256, 'M', 512, 512, 512, 'M', 1024, 1024, 1024, 'A'],
    'VGG16long': [64, 64, 'M', 128, 128, 'M', 256, 256, 256, 'M', 512, 512, 512, 'M', 512, 512, 512, 'M', 1024, 1024, 1024, 'A'],
    ...
    'AllConvImgNet': [128, 128, 128, 'M', 256, 256, 256, 'M', 512, 512, 512, 'M', 512, 512, 512, 'M', 1024, 1024, 1000, 'A'],
}
    ...
    def _make_layers(self, cfg, use_bias, use_bn, do_topdown, do_countpath):
        ...
        for i, x in enumerate(cfg):
            if x == 'M':
                ...
                if do_topdown:
                    if use_bn:
                        layers_drm += [UpsampleLayer(size=upsize, scale=1./(upsize**2), prefix='avg_'), nn.BatchNorm()]
                    else:
                        layers_drm += [UpsampleLayer(size=upsize, scale=1./(upsize**2), prefix='avg_')]
                        
                if do_countpath:
                    if use_bn:
                        layers_drm_cp += [UpsampleLayer(size=upsize, scale=1./(upsize**2), prefix='avg_'), nn.BatchNorm()]
                    else:
                        layers_drm_cp += [UpsampleLayer(size=upsize, scale=1./(upsize**2), prefix='avg_')]
            else:
                ...
                if use_bn:
                    if self.vgg_name == 'AllConvImgNet' and (x == 1024 or x == 1000) and cfg[i-1] != 'M':
                        conv_layer = nn.Conv2D(in_channels=in_channels, channels=x, kernel_size=(1,1), padding=padding_fw, use_bias=False)
                    else:
                    ...
                    if do_topdown:
                        if (cfg[i-1] == 'M' or cfg[i-1] == 'A') and not i == 0:
                            if self.vgg_name == 'AllConvImgNet' and (x == 1024 or x == 1000) and cfg[i-1] != 'M':
                                layers_drm += [nn.Conv2DTranspose(channels=in_channels, in_channels=x, kernel_size=1, strides=(1, 1),
                            ...
                            if self.vgg_name == 'AllConvImgNet' and (x == 1024 or x == 1000) and cfg[i-1] != 'M':
                                layers_drm += [nn.BatchNorm(), nn.Conv2DTranspose(channels=in_channels, in_channels=x, kernel_size=1, strides=(1, 1),
                                                              padding=padding_bw, use_bias=False, params=conv_layer.params)]
                            else:
                    ...
                    if do_countpath:
                        if cfg[i-1] == 'M' or cfg[i-1] == 'A':
                            if self.vgg_name == 'AllConvImgNet' and (x == 1024 or x == 1000) and cfg[i-1] != 'M':
                                layers_drm_cp += [nn.Conv2DTranspose(channels=in_channels, in_channels=x, kernel_size=1, strides=(1, 1),
                            ...
                            if self.vgg_name == 'AllConvImgNet' and (x == 1024 or x == 1000) and cfg[i-1] != 'M':
                                layers_drm_cp += [nn.BatchNorm(), nn.Conv2DTranspose(channels=in_channels, in_channels=x, kernel_size=1, strides=(1, 1),
                                                       padding=padding_bw, use_bias=False)]
                            else:
                    ...
                    if self.vgg_name == 'AllConvImgNet' and (x == 1024 or x == 1000) and cfg[i-1] != 'M':
                        conv_layer = nn.Conv2D(in_channels=in_channels, channels=x, kernel_size=(1,1), padding=padding_fw, use_bias=True)
                    else:
        ...
        for i in range(len(net._children)):
            ...
            if (net._children[i].name.find('conv') != -1) and (i != (len(net._children)-2)):
                if self.do_nn and self.use_bn:
                    loss_nn = loss_nn + 0.5*F.mean(((F.mean(mu, axis=1, exclude=True) - meanfw[meanvar_indx])**2)/(varfw[meanvar_indx] + 1e-8)) + 0.5*F.mean(F.mean((mu - F.mean(mu, axis=1, exclude=True, keepdims=True))**2, axis=1, exclude=True)/(varfw[meanvar_indx] + 1e-8)) - 0.5*F.mean(F.log(F.mean((mu - F.mean(mu, axis=1, exclude=True, keepdims=True))**2, axis=1, exclude=True)+ + 1e-8) - F.log(varfw[meanvar_indx] + 1e-8)) - 0.5
                    #if loss_nn[0] > 1000000:
