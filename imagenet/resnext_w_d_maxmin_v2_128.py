...
def get_mean_act(F, x, keepdims=False):
    if keepdims:
        return F.mean(F.mean(F.mean(x, axis=0, keepdims=keepdims), axis=2, keepdims=keepdims), axis=3, keepdims=keepdims)
    else:
        return F.mean(F.mean(F.mean(x, axis=0), axis=1), axis=1)
        
def compute_kl_gaussian(F, x, meanfw, meanvar_indx):
    out = F.mean((get_mean_act(F,x) - meanfw[meanvar_indx])**2)
...
class residual_unit(nn.HybridBlock):
    ...
    def hybrid_forward(self, F, xmax, xmin):
        ...
        meanfwmax.append(get_mean_act(F, xmax))
        ...
        meanfwmin.append(get_mean_act(F, xmin))
        ...
        return xmax, xmin, ahatmax, ahatmin, meanfwmax, meanfwmin
...
class topdown_residual_unit(nn.HybridBlock):
    ...
    def hybrid_forward(self, F, xmax, xmin, ahatmax, ahatmin, meanfwmax, meanfwmin):
        if not self.dim_match:
            loss_mmmax = compute_kl_gaussian(F, xmax, meanfwmax, 0)
            loss_mmmax = loss_mmmax + compute_kl_gaussian(F, xmax, meanfwmax, 0)
        else:
            loss_mmmax = compute_kl_gaussian(F, xmax, meanfwmax, 0)
        ...
        if not self.dim_match:
            loss_mmmin = compute_kl_gaussian(F, xmin, meanfwmin, 0) 
            loss_mmmin = loss_mmmin + compute_kl_gaussian(F, xmin, meanfwmin, 0)
        else:
            loss_mmmin = compute_kl_gaussian(F, xmin, meanfwmin, 0)
        ...
        return xmax, xmin, loss_mmmax, loss_mmmin
...
class resnext(nn.HybridBlock):
    ...
    def __init__(self, units, num_stage, filter_list, ratio_list, num_class, num_group, data_type, drop_out, bn_mom=0.9, **kwargs):
        ...
        self.bnfirst = nn.BatchNorm(in_channels=3, epsilon=2e-5, momentum=bn_mom, prefix='batchnormfirst_')
        ...
        self.bntdfinal = nn.BatchNorm(in_channels=3, epsilon=2e-5, momentum=bn_mom, prefix='td_batchnormfinal_')
        self.bntdfinalmin = nn.BatchNorm(in_channels=3, epsilon=2e-5, momentum=bn_mom, prefix='td_batchnormfinalmin_')
        ...
        self.upsample1 = UpsampleLayer(size=4, scale=1./(4**2), prefix='up1_')
    ...
    def hybrid_forward(self, F, x, y, cond):
        ...
        x = self.conv0(xbn)
        meanfwmax.append(get_mean_act(F, x))
        ...
        for res_unit in self.residual_stages:
            xmax, xmin, ahatmax_res, ahatmin_res, meanfwmax_res, meanfwmin_res = res_unit(xmax, xmin)
        ...
        xhatmax, xhatmin, loss_mmmax, loss_mmmin = self.topdown(F, self.topdown_stages, xmu, ahatmax, ahatmin, thatmax, thatmin, meanfwmax, meanfwmin)
        
        del ahatmax, ahatmin, thatmax, thatmin, meanfwmax, meanfwmin
        
        return xmax, xmin, xhatmax, xhatmin, loss_mmmax, loss_mmmin, xbn
    
    def topdown(self, F, net, x, ahatmax, ahatmin, thatmax, thatmin, meanfwmax, meanfwmin):
        #x = F.cast(x, 'float16')
        ...
        for i, (td_unit, amax, amin, meanmax, meanmin) in enumerate(zip(net, ahatmax[0:-1], ahatmin[0:-1], meanfwmax[0:-1], meanfwmin[0:-1])):
            xmax, xmin, loss_mmmax_res, loss_mmmin_res = td_unit(xmax, xmin, amax, amin, meanmax, meanmin)
            if i == 0:
        ...
        loss_mmmax = loss_mmmax + compute_kl_gaussian(F, xmax, meanfwmax, -1)
        xmax = self.bntd0(xmax)
        
        loss_mmmin = loss_mmmin + compute_kl_gaussian(F, xmin, meanfwmin, -1)
        ...
        xmax = self.bntdfinal(xmax)
        xmin = self.bntdfinalmin(xmin)
