...
class residual_unit(nn.HybridBlock):
    ...
    def __init__(self, in_channels, num_filter, ratio, strides, dim_match, name, num_group,  bn_mom=0.9, **kwargs):
        ...
        self.in1 = nn.BatchNorm(in_channels=int(num_filter*0.5), epsilon=2e-5, momentum=bn_mom, prefix=name + '_insnorm1_')
        self.bn1min = nn.BatchNorm(in_channels=int(num_filter*0.5), epsilon=2e-5, momentum=bn_mom, prefix=name + '_batchnorm1min_')
        self.in1min = nn.BatchNorm(in_channels=int(num_filter*0.5), epsilon=2e-5, momentum=bn_mom, prefix=name + '_insnorm1min_')
        ...
        self.in2 = nn.BatchNorm(in_channels=int(num_filter*0.5), epsilon=2e-5, momentum=bn_mom, prefix=name + '_insnorm2_')
        self.bn2min = nn.BatchNorm(in_channels=int(num_filter*0.5), epsilon=2e-5, momentum=bn_mom, prefix=name + '_batchnorm2min_')
        self.in2min = nn.BatchNorm(in_channels=int(num_filter*0.5), epsilon=2e-5, momentum=bn_mom, prefix=name + '_insnorm2min_')
        ...
        self.in3 = nn.BatchNorm(in_channels=num_filter, epsilon=2e-5, momentum=bn_mom, prefix=name + '_insnorm3_')
        self.bn3min = nn.BatchNorm(in_channels=num_filter, epsilon=2e-5, momentum=bn_mom, prefix=name + '_batchnorm3min_')
        self.in3min = nn.BatchNorm(in_channels=num_filter, epsilon=2e-5, momentum=bn_mom, prefix=name + '_insnorm3min_')
        ...
        if not dim_match:
            ...
            self.in_sc = nn.BatchNorm(in_channels=num_filter, epsilon=2e-5, momentum=bn_mom, prefix=name + '_sc_insnorm_')
            self.bn_scmin = nn.BatchNorm(in_channels=num_filter, epsilon=2e-5, momentum=bn_mom, prefix=name + '_sc_batchnormmin_')
            self.in_scmin = nn.BatchNorm(in_channels=num_filter, epsilon=2e-5, momentum=bn_mom, prefix=name + '_sc_insnormmin_')
    ...
    def hybrid_forward(self, F, xmax, xmin):
        ...
        ximax = F.slice_axis(xmax, axis=0, begin=0, end=-1)
        meanfwmax.append(F.mean(ximax, axis=1, exclude=True))
        varfwmax.append(F.mean((ximax - F.broadcast_like(F.mean(ximax, axis=1, exclude=True, keepdims=True),ximax, lhs_axes=(0,2,3)))**2, axis=1, exclude=True))

        xbmax = F.slice_axis(xmax, axis=0, begin=-1, end=None)
        ximax = self.bn1(ximax)
        xbmax = self.in1(xbmax)
        xmax = F.concat(ximax, xbmax, dim=0)
        ...
        ximin = F.slice_axis(xmin, axis=0, begin=0, end=-1)
        meanfwmin.append(F.mean(ximin, axis=1, exclude=True))
        varfwmin.append(F.mean((ximin - F.broadcast_like(F.mean(ximin, axis=1, exclude=True, keepdims=True),ximin, lhs_axes=(0,2,3)))**2, axis=1, exclude=True))
        
        xbmin = F.slice_axis(xmin, axis=0, begin=-1, end=None)
        ximin = self.bn1min(ximin)
        xbmin = self.in1min(xbmin)
        xmin = F.concat(ximin, xbmin, dim=0)
        ...
        xbmax = F.slice_axis(xmax, axis=0, begin=-1, end=None)
        ximax = self.bn2(ximax)
        xbmax = self.in2(xbmax)
        xmax = F.concat(ximax, xbmax, dim=0)
        ...
        xbmin = F.slice_axis(xmin, axis=0, begin=-1, end=None)
        ximin = self.bn2min(ximin)
        xbmin = self.in2min(xbmin)
        xmin = F.concat(ximin, xbmin, dim=0)
        ...
        xbmax = F.slice_axis(xmax, axis=0, begin=-1, end=None)
        ximax = self.bn3(ximax)
        xbmax = self.in3(xbmax)
        xmax = F.concat(ximax, xbmax, dim=0)
        ...
        xbmin = F.slice_axis(xmin, axis=0, begin=-1, end=None)
        ximin = self.bn3min(ximin)
        xbmin = self.in3min(xbmin)
        xmin = F.concat(ximin, xbmin, dim=0)
        ...
        if not self.dim_match:
            ...
            shortcutimax = F.slice_axis(shortcutmax, axis=0, begin=0, end=-1)
            meanfwmax.append(F.mean(shortcutimax, axis=1, exclude=True))
            varfwmax.append(F.mean((shortcutimax - F.broadcast_like(F.mean(shortcutimax, axis=1, exclude=True, keepdims=True),shortcutimax, lhs_axes=(0,2,3)))**2, axis=1, exclude=True))
            
            shortcutbmax = F.slice_axis(shortcutmax, axis=0, begin=-1, end=None)
            shortcutimax = self.bn_sc(shortcutimax)
            shortcutbmax = self.in_sc(shortcutbmax)
            shortcutmax = F.concat(shortcutimax, shortcutbmax, dim=0)
            
            shortcutmax = self.bias_sc(shortcutmax)
            bfw.append(self.bias_sc.bias)
            
            shortcutmin = self.fc_sc(shortcutmin)
            shortcutimin = F.slice_axis(shortcutmin, axis=0, begin=0, end=-1)
            meanfwmin.append(F.mean(shortcutimin, axis=1, exclude=True))
            varfwmin.append(F.mean((shortcutimin - F.broadcast_like(F.mean(shortcutimin, axis=1, exclude=True, keepdims=True),shortcutimin, lhs_axes=(0,2,3)))**2, axis=1, exclude=True))
            
            shortcutbmin = F.slice_axis(shortcutmin, axis=0, begin=-1, end=None)
            shortcutimin = self.bn_scmin(shortcutimin)
            shortcutbmin = self.in_scmin(shortcutbmin)
            shortcutmin = F.concat(shortcutimin, shortcutbmin, dim=0)
...
class topdown_residual_unit(nn.HybridBlock):
    ...
    def __init__(self, fwblock, name, **kwargs):
        ...
        self.in3 = nn.BatchNorm(in_channels=self.num_filter, epsilon=2e-5, momentum=self.bn_mom, prefix=name + '_td_insnorm3_')
        self.bn3min = nn.BatchNorm(in_channels=self.num_filter, epsilon=2e-5, momentum=self.bn_mom, prefix=name + '_td_batchnorm3min_')
        self.in3min = nn.BatchNorm(in_channels=self.num_filter, epsilon=2e-5, momentum=self.bn_mom, prefix=name + '_td_insnorm3min_')
        ...
        self.in2 = nn.BatchNorm(in_channels=int(self.num_filter*0.5), epsilon=2e-5, momentum=self.bn_mom, prefix=name + '_td_insnorm2_')
        ...
        self.in2min = nn.BatchNorm(in_channels=int(self.num_filter*0.5), epsilon=2e-5, momentum=self.bn_mom, prefix=name + '_td_insnorm2min_')
        ...
        self.in1 = nn.BatchNorm(in_channels=int(self.num_filter*0.5), epsilon=2e-5, momentum=self.bn_mom, prefix=name + '_td_insnorm1_')
        ...
        self.in1min = nn.BatchNorm(in_channels=int(self.num_filter*0.5), epsilon=2e-5, momentum=self.bn_mom, prefix=name + '_td_insnorm1min_')
    ...
    def hybrid_forward(self, F, xmax, xmin, ahatmax, ahatmin, meanfwmax, varfwmax, meanfwmin, varfwmin, bfw):
        ...
        if not self.dim_match:
            loss_mmmax = compute_kl_gaussian(F, ximax, meanfwmax, varfwmax, 0)
            loss_mmmax = loss_mmmax + compute_kl_gaussian(F, ximax, meanfwmax, varfwmax, 0)
        else:
            loss_mmmax = compute_kl_gaussian(F, ximax, meanfwmax, varfwmax, 0)
        ...
        xbmin = F.slice_axis(xmin, axis=0, begin=-1, end=None)
        if not self.dim_match:
            loss_mmmin = compute_kl_gaussian(F, ximin, meanfwmin, varfwmin, 0) 
            loss_mmmin = loss_mmmin + compute_kl_gaussian(F, ximin, meanfwmin, varfwmin, 0)
        else:
            loss_mmmin = compute_kl_gaussian(F, ximin, meanfwmin, varfwmin, 0)
        ...
        ximax = F.slice_axis(xmax, axis=0, begin=0, end=-1)
        xbmax = F.broadcast_like(F.slice_axis(xmax, axis=0, begin=-1, end=None), ximax, lhs_axes=(0,2,3))
        if not self.dim_match:
            rpnmax = F.mean(F.abs((ximax-xbmax)*F.broadcast_like(bfw[0].var(), ximax, lhs_axes=(0,2,3))), axis=0, exclude=True) 
            rpnmax = rpnmax + F.mean(F.abs((ximax-xbmax)*F.broadcast_like(bfw[1].var(), ximax, lhs_axes=(0,2,3))), axis=0, exclude=True)
        else:
            rpnmax = F.mean(F.abs((ximax-xbmax)*F.broadcast_like(bfw[0].var(), ximax, lhs_axes=(0,2,3))), axis=0, exclude=True)
        
        ximin = F.slice_axis(xmin, axis=0, begin=0, end=-1)
        xbmin = F.broadcast_like(F.slice_axis(xmin, axis=0, begin=-1, end=None), ximin, lhs_axes=(0,2,3))
        if not self.dim_match:
            rpnmin = F.mean(F.abs((ximin-xbmin)*F.broadcast_like(bfw[0].var(), ximin, lhs_axes=(0,2,3))), axis=0, exclude=True) 
            del bfw[0]
            rpnmin = rpnmin + F.mean(F.abs((ximin-xbmin)*F.broadcast_like(bfw[0].var(), ximin, lhs_axes=(0,2,3))), axis=0, exclude=True)
            del bfw[0]
        else:
            rpnmin = F.mean(F.abs((ximin-xbmin)*F.broadcast_like(bfw[0].var(), ximin, lhs_axes=(0,2,3))), axis=0, exclude=True)
            del bfw[0]
        ...
        ximax = F.slice_axis(xmax, axis=0, begin=0, end=-1)
        xbmax = F.broadcast_like(F.slice_axis(xmax, axis=0, begin=-1, end=None), ximax, lhs_axes=(0,2,3))
        rpnmax = rpnmax + F.mean(F.abs((ximax-xbmax)*F.broadcast_like(bfw[0].var(), ximax, lhs_axes=(0,2,3))), axis=0, exclude=True)
        ...
        return xmax, xmin, loss_mmmax, loss_mmmin, rpnmax, rpnmin
...
class resnext(nn.HybridBlock):
    ...
    def __init__(self, units, num_stage, filter_list, ratio_list, num_class, num_group, data_type, drop_out, bn_mom=0.9, **kwargs):
        ...
        self.in0 = nn.BatchNorm(in_channels=filter_list[0], epsilon=2e-5, momentum=bn_mom, prefix='insnorm0_')
        ...
        self.intd0 = nn.BatchNorm(in_channels=filter_list[0], epsilon=2e-5, momentum=bn_mom, prefix='td_insnorm0_')
        self.bntd0min = nn.BatchNorm(in_channels=filter_list[0], epsilon=2e-5, momentum=bn_mom, prefix='td_batchnorm0min_')
        self.intd0min = nn.BatchNorm(in_channels=filter_list[0], epsilon=2e-5, momentum=bn_mom, prefix='td_insnorm0min_')
    ...
    def hybrid_forward(self, F, x, y, cond):
        ...
        xb = F.slice_axis(F.zeros_like(x), axis=0, begin=-1, end=None)
        x = F.concat(x, xb, dim=0)
        
        x = self.conv0(x)
        xi = F.slice_axis(x, axis=0, begin=0, end=-1)
        meanfwmax.append(F.mean(xi, axis=1, exclude=True))
        varfwmax.append(F.mean((xi - F.broadcast_like(F.mean(xi, axis=1, exclude=True, keepdims=True), xi, lhs_axes=(0,2,3)))**2, axis=1, exclude=True))
        meanfwmin.append(meanfwmax[-1])
        varfwmin.append(varfwmax[-1])

        xb = F.slice_axis(x, axis=0, begin=-1, end=None)
        xi = self.bn0(xi)
        xb = self.in0(xb)
        x = F.concat(xi, xb, dim=0)
        ...
        ci = F.where(cond, y, F.argmax(xi, axis=1))
        cb = F.argmax(xb, axis=1)
        
        xi = F.one_hot(ci, self.num_class)
        xb = F.slice_axis(F.ones_like(xi), axis=0, begin=-1, end=None)
        x = F.concat(xi, xb, dim=0)
        
        xhatmax, xhatmin, loss_mmmax, loss_mmmin, rpnmax, rpnmin = self.topdown(F, self.topdown_stages, x, ahatmax, ahatmin, thatmax, thatmin, meanfwmax, meanfwmin, varfwmax, varfwmin, bfw)
        
        del ahatmax, ahatmin, thatmax, thatmin, meanfwmax, meanfwmin, varfwmax, varfwmin, bfw
        
        xihatmax = F.slice_axis(xhatmax, axis=0, begin=0, end=-1)
        xihatmin = F.slice_axis(xhatmin, axis=0, begin=0, end=-1)
        
        return ximax, ximin, xihatmax, xihatmin, loss_mmmax, loss_mmmin, rpnmax, rpnmin
    ...
    def topdown(self, F, net, x, ahatmax, ahatmin, thatmax, thatmin, meanfwmax, meanfwmin, varfwmax, varfwmin, bfw):
        ...
        for i, (td_unit, amax, amin, meanmax, meanmin, varmax, varmin, b) in enumerate(zip(net, ahatmax[0:-1], ahatmin[0:-1], meanfwmax[0:-1], meanfwmin[0:-1], varfwmax[0:-1], varfwmin[0:-1], bfw[0:-1])):
            xmax, xmin, loss_mmmax_res, loss_mmmin_res, rpnmax_res, rpnmin_res = td_unit(xmax, xmin, amax, amin, meanmax, meanmin, varmax, varmin, b)
            if i == 0:
                ...
                rpnmax = rpnmax + rpnmax_res
                rpnmin = rpnmin + rpnmin_res
        
        ximax = F.slice_axis(xmax, axis=0, begin=0, end=-1)
        xbmax = F.slice_axis(xmax, axis=0, begin=-1, end=None)
        loss_mmmax = loss_mmmax + compute_kl_gaussian(F, ximax, meanfwmax, varfwmax, -1)
        ximax = self.bntd0(ximax)
        xbmax = self.intd0(xbmax)
        xmax = F.concat(ximax, xbmax, dim=0)
        
        ximin = F.slice_axis(xmin, axis=0, begin=0, end=-1)
        xbmin = F.slice_axis(xmin, axis=0, begin=-1, end=None)
        loss_mmmin = loss_mmmin + compute_kl_gaussian(F, ximin, meanfwmin, varfwmin, -1)
        ximin = self.bntd0min(ximin)
        xbmin = self.intd0min(xbmin)
        xmin = F.concat(ximin, xbmin, dim=0)
        ...
        ximin = F.slice_axis(xmin, axis=0, begin=0, end=-1)
        xbmin = F.broadcast_like(F.slice_axis(xmin, axis=0, begin=-1, end=None), ximin, lhs_axes=(0,2,3))
        rpnmin = rpnmin + F.mean(F.abs((ximin-xbmin)*F.broadcast_like(bfw[-1].var(), ximin, lhs_axes=(0,2,3))), axis=0, exclude=True)
        del bfw[-1]
